...
- The Hermes JSON-RPC client speaks the MIT dashboard contract (`GET /api/status`, `/api/ws` on port 9119, `session.create`, `prompt.submit`). Hermes CORS only allows localhost origins, and `/api/ws` still needs the dashboard session token. This environment has no running Hermes dashboard. A public HTTPS preview cannot reach Hermes on your laptop. OAuth single-use tickets are not implemented. Demo messages stay queued.
