...
describe("editorial surface CSS contract", () => {
  ...
  it("defines shared bottom chrome and a theme-compatible featured surface", () => {
    ...
    expect(globalsCss).toMatch(/--mk-safe-top:\s*env\(safe-area-inset-top/)
    expect(globalsCss).toMatch(/--mk-safe-right:\s*env\(safe-area-inset-right/)
    expect(globalsCss).toMatch(/--mk-safe-left:\s*env\(safe-area-inset-left/)
